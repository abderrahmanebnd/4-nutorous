...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) {
    return next(new AppError('Tou are not logged in!', 401));
  }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  // here the resetToken is the original one not the encrypted one
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  user.passwordResetToken = undefined;
  user.passwordResetExpires = undefined;
  // here we need to use save() instead of findOneAndUpdate() because we need to run the validators
  await user.save();
